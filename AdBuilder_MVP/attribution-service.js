...
const EventUTM = require('../../models/EventUTM');
...
class AttributionService {
   ...
   * @param {String} source - Traffic source (default: meta)
   * @param {String} medium - Marketing medium (default: paid)
   * @param {String} content - Ad content identifier (optional)
   * @returns {Object} UTM parameters
   */
  static generateUTM(campaignId, source = 'meta', medium = 'paid', content = null) {
    ...
    const utm = {
      utm_source: source,
      utm_medium: medium,
      utm_campaign: campaign.attribution.utm_campaign || `sonik_${campaignId}`,
      utm_content: content || campaign._id.toString(),
      utm_term: null // Reserved for search keywords
    };
    ...
    return {
      parameters: utm,
      queryString: utmString,
      fullUrl: `https://sonik.app/events/${campaign.eventId}?${utmString}`
    };
  }
   ...
   * @param {Object} metadata - Additional metadata (page, referrer, etc.)
   * @returns {Promise} Tracking result
   */
  static async trackPageView(utmParams, sessionId, visitorId, metadata = {}) {
    const session = mongoose.startSession();
    
    try {
      await session.withTransaction(async () => {
        // Extract campaign ID from UTM
        const campaignIdentifier = utmParams.utm_campaign;
        if (!campaignIdentifier) return;

        // Find campaign by UTM campaign name
        const campaign = await Campaign.findOne({
          'attribution.utm_campaign': campaignIdentifier
        }).session(session);

        if (!campaign) return;

        // Create or update EventUTM record
        const eventUTM = await EventUTM.findOneAndUpdate(
          {
            _event: campaign.eventId,
            utm_campaign: campaignIdentifier,
            sessionId: sessionId
          },
          {
            $set: {
        ...
        // Update campaign view metrics
        await Campaign.findByIdAndUpdate(
          campaign._id,
          {
            $inc: {
              'ticketAttribution.views': 1
            },
            $addToSet: {
              'ticketAttribution.uniqueVisitors': visitorId
            }
          },
          { session }
        );

        // Store in Redis for quick lookup (24 hour TTL)
        const cacheKey = `attribution:session:${sessionId}`;
        await RedisClient.setex(
          cacheKey,
          86400, // 24 hours
          JSON.stringify({
            campaignId: campaign._id.toString(),
            eventUTMId: eventUTM._id.toString(),
            utmParams,
            timestamp: new Date()
          })
        );
        ...
        await this._queueAnalyticsEvent('page_view', {
   ...
   * @param {Number} cartValue - Cart value in cents
   * @param {String} sessionId - User session ID
   * @param {Object} items - Cart items
   * @returns {Promise} Tracking result
   */
  static async trackAddToCart(campaignId, eventId, cartValue, sessionId, items = []) {
    const session = mongoose.startSession();

    try {
      await session.withTransaction(async () => {
        // Get attribution data from cache
        const cacheKey = `attribution:session:${sessionId}`;
        const attributionData = await RedisClient.get(cacheKey);
        
        if (!attributionData) {
          // No attribution data found, skip tracking
          return;
        }

        const { campaignId: attributedCampaignId } = JSON.parse(attributionData);
        
        // Only track if the campaign matches
        if (attributedCampaignId !== campaignId) {
          return;
        }
        ...
        await Campaign.findByIdAndUpdate(
          ...
          {
            $inc: {
              'ticketAttribution.addedToCart': 1,
              'ticketAttribution.cartValue': cartValue
            }
          },
          { session }
        );

        // Store cart data in Redis for conversion tracking (1 hour TTL)
        const cartKey = `attribution:cart:${sessionId}`;
        await RedisClient.setex(
          cartKey,
          3600, // 1 hour
          JSON.stringify({
        ...
        await this._queueAnalyticsEvent('add_to_cart', {
          campaignId,
          eventId,
          sessionId,
          cartValue,
  ...
  /**
   * Track purchase and attribute to campaign
   * @param {String} orderId - Order/TicketTransaction ID
   * @param {String} campaignId - Campaign ID (optional, will be detected from session)
   * @param {String} customerId - Customer ID
   * @param {String} sessionId - Session ID
   * @returns {Promise} Attribution result
   */
  static async trackPurchase(orderId, campaignId, customerId, sessionId) {
    const session = mongoose.startSession();
    ...
    try {
      ...
      await session.withTransaction(async () => {
        ...
        const order = await TicketTransaction.findById(orderId).session(session);
        if (!order) {
        ...
        if (!attributionData) {
          // No attribution data in session, cannot attribute
          return;
        }
        ...
        } = JSON.parse(attributionData);

        // Use attributed campaign ID if not provided
        campaignId = campaignId || attributedCampaignId;

        // Verify campaign exists and matches event
        const campaign = await Campaign.findById(campaignId).session(session);
        if (!campaign || campaign.eventId.toString() !== order._event.toString()) {
          return;
        }
        ...
        const timeToConversion = Math.floor((purchaseTime - viewTime) / 1000 / 60); // in minutes

        // Update order with attribution data
        order._eventUTM = eventUTMId;
        order.source = utmParams.utm_source || 'meta';
        await order.save({ session });

        // Calculate order revenue (total amount)
        const revenue = order.priceBreakdown.total;

        // Update campaign attribution metrics
        await Campaign.findByIdAndUpdate(
          campaignId,
          {
            $inc: {
              'ticketAttribution.purchases': 1,
              'ticketAttribution.revenue': revenue
            },
            $push: {
              'ticketAttribution.customers': {
                customerId,
                purchaseDate: purchaseTime,
                amount: revenue,
                ticketTypes: order.ticketItems.map(item => item._tickettier.toString())
              }
            }
          },
          { session }
        ...
        // Update EventUTM conversion tracking
        await EventUTM.findByIdAndUpdate(
          eventUTMId,
          {
            $inc: { conversions: 1, conversionValue: revenue },
            $set: { lastConvertedAt: purchaseTime }
          },
          { session }
        );

        // Clear attribution cache after successful conversion
        await RedisClient.del(cacheKey);
        await RedisClient.del(`attribution:cart:${sessionId}`);

        // Queue analytics event
        await this._queueAnalyticsEvent('purchase', {
          campaignId,
          eventId: order._event,
          orderId,
          customerId,
          revenue,
          timeToConversion,
  ...
  /**
   * Get conversion path for a customer
   * @param {String} customerId - Customer ID
   * @param {String} campaignId - Campaign ID (optional)
   * @returns {Promise<Array>} Conversion path events
   */
  static async getConversionPath(customerId, campaignId = null) {
    try {
      // Find all orders by customer
      const orderFilter = { _user: customerId };
      if (campaignId) {
        // First get campaign to find event
        const campaign = await Campaign.findById(campaignId);
        if (campaign) {
          orderFilter._event = campaign.eventId;
        }
      }

      const orders = await TicketTransaction.find(orderFilter)
        .populate('_eventUTM')
        .sort({ createdAt: 1 });
      ...
      for (const order of orders) {
        if (order._eventUTM) {
          // Get campaign for this UTM
          const campaign = await Campaign.findOne({
            'attribution.utm_campaign': order._eventUTM.utm_campaign
          });

          if (campaign && (!campaignId || campaign._id.toString() === campaignId)) {
            path.push({
              ...
              revenue: order.priceBreakdown.total,
              source: order._eventUTM.utm_source,
              medium: order._eventUTM.utm_medium
            });
          }
        }
      }
  ...
  /**
   * Calculate attribution window for an interaction
   * @param {Date} interactionTime - Time of interaction (click/view)
   * @param {Date} conversionTime - Time of conversion
   * @param {Object} windowSettings - Attribution window settings
   * @returns {Boolean} Whether conversion is within attribution window
   */
  static calculateAttributionWindow(
    interactionTime, 
    conversionTime, 
    windowSettings = { clickDays: 7, viewDays: 1 }
  ) {
    const timeDiff = conversionTime - interactionTime;
    const daysDiff = timeDiff / (1000 * 60 * 60 * 24);

    // For now, we'll assume all interactions are clicks
    // In the future, we'd differentiate between clicks and views
    return daysDiff <= windowSettings.clickDays && daysDiff >= 0;
  }

  /**
   * Queue analytics event for processing
   * @private
   */
  static async _queueAnalyticsEvent(eventType, data) {
    try {
      // In production, this would push to a queue like Bull
      // For now, we'll just log it
      console.log('Analytics Event:', eventType, data);
      
      // Store in Redis for batch processing
      const key = `analytics:queue:${eventType}`;
   ...
   */
  static async getCampaignAttributionReport(campaignId, filters = {}) {
    try {
      const campaign = await Campaign.findById(campaignId);
      if (!campaign) {
        throw new Error('Campaign not found');
      ...
      const orders = await TicketTransaction.find({
        _event: campaign.eventId,
        '_eventUTM': { $exists: true }
      }).populate('_eventUTM _user');

      // Filter orders by UTM campaign
      const attributedOrders = orders.filter(order => 
        order._eventUTM && order._eventUTM.utm_campaign === campaign.attribution.utm_campaign
      );

      // Apply date filters if provided
      let filteredOrders = attributedOrders;
      if (filters.startDate || filters.endDate) {
        filteredOrders = attributedOrders.filter(order => {
      ...
      const report = {
        ...
        totalRevenue: filteredOrders.reduce((sum, order) => sum + order.priceBreakdown.total, 0),
        averageOrderValue: 0,
        conversionsByDay: {},
        conversionsByDevice: {},
        topCustomers: [],
      ...
      if (filteredOrders.length > 0) {
        ...
        filteredOrders.forEach(order => {
          ...
          if (!report.conversionsByDay[day]) {
            report.conversionsByDay[day] = {
              orders: 0,
              revenue: 0
            };
          }
          report.conversionsByDay[day].orders++;
          report.conversionsByDay[day].revenue += order.priceBreakdown.total;

          // Device tracking (would need to be implemented in EventUTM)
          const device = order._eventUTM.metadata?.device || 'unknown';
          if (!report.conversionsByDevice[device]) {
            report.conversionsByDevice[device] = 0;
          }
          report.conversionsByDevice[device]++;
        });

        // Top customers
        const customerRevenue = {};
        filteredOrders.forEach(order => {
          const customerId = order._user._id.toString();
          if (!customerRevenue[customerId]) {
            customerRevenue[customerId] = {
              customer: order._user,
              revenue: 0,
              orders: 0
            };
          }
          customerRevenue[customerId].revenue += order.priceBreakdown.total;
          customerRevenue[customerId].orders++;
        });

        report.topCustomers = Object.values(customerRevenue)
...
const attributionMiddleware = async (req, res, next) => {
  try {
    ...
    let sessionId = req.session?.id || req.cookies?.sonik_session;
    if (!sessionId) {
      sessionId = uuidv4();
      res.cookie('sonik_session', sessionId, {
        httpOnly: true,
    ...
    let visitorId = req.cookies?.sonik_visitor;
    if (!visitorId) {
      visitorId = uuidv4();
      res.cookie('sonik_visitor', visitorId, {
        httpOnly: true,
        secure: process.env.NODE_ENV === 'production',
        sameSite: 'lax',
    ...
    AttributionService.trackPageView(utmParams, sessionId, visitorId, {
      page: req.path,
    ...
    if (req.session) {
      req.session.utmParams = utmParams;
      req.session.attributionTimestamp = new Date();
    }

    next();
  } catch (error) {
    console.error('Attribution middleware error:', error);
...
const getAttributionData = (req) => {
  const sessionId = req.session?.id || req.cookies?.sonik_session;
  const visitorId = req.cookies?.sonik_visitor;
  const utmParams = req.session?.utmParams || {};

  return {
    sessionId,
    visitorId,
    utmParams,
    hasAttribution: Object.values(utmParams).some(value => value)
  };
};
...
// =====================================
// src/routes/adbuilder/campaign.routes.js (additions)
// =====================================
// Add these routes to the existing campaign.routes.js file

/**
 * @route GET /api/v1/advertising/campaigns/:id/attribution
 * @desc Get detailed attribution data for a campaign
    ...
    try {
      ...
      if (!campaign) {
        return res.status(404).json(
          ResponseFormatter.error(new Error('Campaign not found'), 404)
        );
      }

      // Build query for attributed orders
      const query = {
        _event: campaign.eventId,
        '_eventUTM': { $exists: true },
        status: 'succeeded'
      ...
      // Get total count
      const total = await TicketTransaction.countDocuments(query);

      // Get paginated results
      const orders = await TicketTransaction.find(query)
        .populate('_eventUTM _user')
        .sort({ [sortBy]: sortOrder === 'desc' ? -1 : 1 })
        .skip((page - 1) * limit)
        .limit(limit);

      // Filter by campaign UTM and format results
      const attributionData = orders
        .filter(order => 
          order._eventUTM && 
          order._eventUTM.utm_campaign === campaign.attribution.utm_campaign
        )
        .map(order => {
          // Calculate time to conversion
          const viewTime = order._eventUTM.createdAt;
          const purchaseTime = order.createdAt;
          const timeToConversion = Math.floor((purchaseTime - viewTime) / 1000 / 60); // minutes

          return {
            _id: order._id,
            customerEmail: order._user.email,
            customerId: order._user._id,
            purchaseDate: order.createdAt,
            ticketCount: order.getTicketCount(),
            revenue: order.priceBreakdown.total,
            timeToConversion,
            device: order._eventUTM.metadata?.device || 'unknown',
            source: order._eventUTM.utm_source,
            medium: order._eventUTM.utm_medium
          };
        });
      ...
      if (search) {
        ...
        filteredData = attributionData.filter(item =>
...
/**
 * @route GET /api/v1/advertising/campaigns/:id/attribution/export
 * @desc Export attribution data as CSV
 * @access Private - requires advertising:campaigns:view permission
 */
router.get('/:id/attribution/export',
  checkPermission('advertising:campaigns:view'),
  async (req, res) => {
    try {
      const { organizationId } = req.user;
      ...
      const report = await AttributionService.getCampaignAttributionReport(id, req.query);
      
      // Convert to CSV format
      const csv = [
        'Customer Email,Purchase Date,Tickets,Revenue,Time to Conversion,Device,Source',
        ...report.data.map(row => 
          `${row.customerEmail},${row.purchaseDate},${row.ticketCount},${row.revenue},${row.timeToConversion},${row.device},${row.source}`
        )
      ].join('\n');

      res.setHeader('Content-Type', 'text/csv');
      res.setHeader('Content-Disposition', `attachment; filename="attribution-${id}-${Date.now()}.csv"`);
      res.send(csv);
    } catch (error) {
      console.error('Export attribution error:', error);
      res.status(500).json(ResponseFormatter.error(error));
    }
  }
);
...
/**
 * @route GET /api/v1/advertising/campaigns/:id/activity
 * @desc Get campaign activity log
 * @access Private - requires advertising:campaigns:view permission
 */
router.get('/:id/activity',
  ...
  async (req, res) => {
    try {
      ...
      // Status changes
      campaign.statusHistory.forEach(change => {
        activities.push({
          type: 'status_change',
          timestamp: change.changedAt,
          description: `Campaign status changed to ${change.status}`,
      ...
      // Metric syncs
      if (campaign.metrics.lastSyncedAt) {
        activities.push({
          type: 'metric_sync',
          timestamp: campaign.metrics.lastSyncedAt,
          description: 'Campaign metrics synchronized with Meta',
          metadata: {
      ...
      // Sync errors
      campaign.metrics.syncErrors?.forEach(error => {
        activities.push({
          type: 'error',
          timestamp: error.timestamp,
          description: `Sync error: ${error.error}`,
          metadata: { resolved: error.resolved }
        });
      });
...
/**
 * @route POST /api/v1/advertising/campaigns/:id/refresh
 * @desc Force refresh campaign metrics from Meta
...
router.post('/:id/refresh',
  ...
  async (req, res) => {
    try {
      ...
      // Queue refresh job (in production, this would use Bull queue)
      ...
      campaign.statusHistory.push({
        ...
        changedAt: new Date(),
        changedBy: userId,
        reason: 'Manual metrics refresh requested'
      });
      await campaign.save();
      ...
      res.json(ResponseFormatter.success(
        { 
          ...
          message: 'Metrics refresh has been queued'
        },
        'Refresh initiated successfully'
      ));
    } catch (error) {
      console.error('Refresh campaign error:', error);
      res.status(500).json(ResponseFormatter.error(error));
    }
  }
);
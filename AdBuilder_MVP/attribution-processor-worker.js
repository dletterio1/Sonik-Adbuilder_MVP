// src/workers/attribution-processor.worker.js
 ...
 * Processes order completions and attributes them to campaigns
 */
class AttributionProcessorWorker {
  constructor() {
    ...
    this.batchSize = parseInt(process.env.ATTRIBUTION_PROCESSING_BATCH_SIZE) || 100;
    this.attributionWindowDays = parseInt(process.env.ATTRIBUTION_WINDOW_DAYS) || 28;
   ...
   */
  setupWorker() {
    // Process attribution jobs
    this.queue.process('process-attribution', this.batchSize, async (job) => {
      return await this.processAttribution(job.data);
    });

    // Process batch attribution
    this.queue.process('batch-attribution', async (job) => {
      return await this.processBatchAttribution(job.data);
    });

    // Queue event handlers
    this.queue.on('completed', (job, result) => {
      console.log(`Attribution job ${job.id} completed:`, result);
    });

    this.queue.on('failed', (job, err) => {
      console.error(`Attribution job ${job.id} failed:`, err);
      this.handleFailedJob(job, err);
    });

    console.log('Attribution processor worker initialized');
  }
  ...
  subscribeToEvents() {
    ...
    EventBus.subscribe('order.refunded', async (event) => {
      await this.handleOrderRefunded(event.payload);
    });

    // Subscribe to ticket scanned events for engagement tracking
    EventBus.subscribe('ticket.scanned', async (event) => {
      await this.handleTicketScanned(event.payload);
    });
  }

  /**
   * Handle order completed event
   * @param {Object} payload - Order event payload
   */
  async handleOrderCompleted(payload) {
    const { orderId, customerId, eventId } = payload;

    try {
      // Check if already processed
      const processedKey = `attribution:processing:${orderId}`;
      const isProcessed = await redis.get(processedKey);
      
      if (isProcessed) {
        console.log(`Order ${orderId} already processed for attribution`);
        return;
      }
      ...
      await this.queue.add('process-attribution', {
        orderId,
        customerId,
        eventId,
        timestamp: new Date()
      ...
      await redis.del(`attribution:processing:${orderId}`);
    }
  }
   ...
   */
  async processAttribution(jobData) {
    const { orderId, customerId, eventId } = jobData;
    ...
    try {
      ...
      await session.withTransaction(async () => {
        ...
        if (!order._eventUTM || !order._eventUTM.utm_campaign) {
          console.log(`Order ${orderId} has no UTM data for attribution`);
          return;
        }
        ...
        const campaign = await Campaign.findOne({
          ...
          eventId: order._event._id
        }).session(session);

        if (!campaign) {
          console.log(`No campaign found for UTM: ${order._eventUTM.utm_campaign}`);
          return;
        }

        // Check attribution window
        const orderDate = new Date(order.createdAt);
        const utmDate = new Date(order._eventUTM.createdAt);
        const daysDiff = (orderDate - utmDate) / (1000 * 60 * 60 * 24);

        if (daysDiff > this.attributionWindowDays) {
          console.log(`Order ${orderId} outside attribution window (${daysDiff} days)`);
          return;
        }
        ...
        const timeToConversion = Math.floor((orderDate - utmDate) / 1000 / 60); // minutes
          ...
          campaign._id,
          {
            ...
            $push: {
              'ticketAttribution.customers': {
                ...
                amount: revenue,
                ticketTypes: order.tickets.map(t => t.tier || 'general')
              }
            }
          },
          { session }
        ...
        result = {
          ...
          ticketCount,
          timeToConversion,
          attributionWindow: `${Math.ceil(daysDiff)}-day`
        };

        // Publish conversion event
        await EventBus.publish('campaign.conversion', {
          campaignId: campaign._id,
          orderId,
          customerId,
          revenue,
        ...
        // Update daily attribution stats in Redis
        await this.updateAttributionStats(campaign._id, revenue, orderDate);
      });

      // Mark attribution as processed
      await EventBus.publish('attribution.processed', {
        orderId,
        result
      });
      ...
      console.error(`Error processing attribution for order ${orderId}:`, error);
      throw error;
    } finally {
   ...
   * @param {Object} payload - Refund event payload
   */
  async handleOrderRefunded(payload) {
    ...
    try {
      await session.withTransaction(async () => {
        ...
        // Update campaign metrics (subtract refund)
        await Campaign.findByIdAndUpdate(
          campaign._id,
          {
            $inc: {
              'ticketAttribution.revenue': -refundAmount,
              'ticketAttribution.refunds': 1
            }
          },
          { session }
        );
      ...
      console.error(`Error processing refund attribution for order ${orderId}:`, error);
    } finally {
      await session.endSession();
    }
  }
  ...
  /**
   * Update daily attribution statistics in Redis
   * @param {String} campaignId - Campaign ID
   * @param {Number} revenue - Revenue amount
   * @param {Date} date - Attribution date
   */
  async updateAttributionStats(campaignId, revenue, date) {
    const dateKey = date.toISOString().split('T')[0];
    const statsKey = `attribution:stats:${campaignId}:${dateKey}`;

    await redis.hincrby(statsKey, 'conversions', 1);
    await redis.hincrby(statsKey, 'revenue', revenue);
  ...
  /**
   * Process batch attribution for historical data
   * @param {Object} jobData - Batch job data
   */
  async processBatchAttribution(jobData) {
    const { startDate, endDate, campaignId } = jobData;
    ...
    try {
      ...
      for (const order of orders) {
        try {
          const result = await this.processAttribution({
            orderId: order._id,
            customerId: order._user,
      ...
      console.error('Error in batch attribution:', error);
      throw error;
    }
  }

  /**
   * Handle failed attribution job
  ...
  async handleFailedJob(job, error) {
    ...
    // Remove processing lock on final failure
    if (job.attemptsMade >= job.opts.attempts) {
      await redis.del(`attribution:processing:${orderId}`);
      
      // Log to error tracking
      console.error(`Attribution permanently failed for order ${orderId}:`, error);
   ...
   */
  start() {
    console.log('Starting attribution processor worker...');
